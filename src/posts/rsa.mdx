...
However, it's important to understand how number representation and arithmetic work in JavaScript and most 
modern programming languages. 
JavaScript uses the **IEEE 754 standard for floating-point arithmetic**, which is widely adopted across 
languages like Python, Java, and C. 
While this format allows for a wide range of numerical operations, it introduces limitations when 
it comes to precision: numbers above approximately 
...
It's crucial to work with very large integers $p$ and $q$: the largest they are the harder is to factorize their product.

To deal with large integeers, JavaScript provides a native **BigInt** type. For example:
...
There are anyway libraries to implement more involved features like modular inverse and so on.
