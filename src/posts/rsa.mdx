...
<CalloutBox type="definition" title="Modular Congruence">
  ...
  $a$ is <strong>congruent</strong> to $b$ modulo $m$ (written as $a \equiv b \pmod m$), if there exists an integer $k$ such that: $a = b + km$.
</CalloutBox>
...
Remeber that $d$ is the modular inverse of $e$. Thus $de \equiv 1 \pmod{\phi(n)}$.
